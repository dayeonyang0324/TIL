...
>
> 자세한 내용은 [SQLite3 공식문서](https://sqlite.org/datatype3.html)
...
- `INTEGER PRIMARY KEY` 타입으로 컬럼 생성시 기본 `row id` 를 대체함
...
- `select * FROM table WHERE 조건`
  - 항상 from 뒤에 위치하며 조건에는 비교연산자 사용하여 구체적인 조건을 준다.
- `select * FROM table GROUP BY 필드 HAVING 조건`
  - 항상 GROUP BY 뒤에 위치하고 조건절은 where과 똑같이 작성된다
- 차이점
  - where은 모든 필드를 조건에 둘 수 있다. having은 group된 특정 필드에만 줄 수 있다.
  - 조건에 맞는 정보를 다 들고와서 group하는것과 그룹화 된 상태에서 조건에 맞는 정보를 걸러내는 것의 차이

<br>

### JOIN

- `A LEFT JOIN B ON 조건(id=id)`
  - A 값은 다 나타나고 일치하는 key값만 추가로 나타남
- `LEFT JOIN (IS NULL)`
  - Null 연결이 없는 값만 나타남
- `INNER JOIN`
  - ID 값이 서로 중복되는 값만 나타남
