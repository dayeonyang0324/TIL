...
2. user 레코드 생성
   ...
   ```sql
   -- sql
   INSERT INTO users_user VALUES (102, '철수', '김', 100, '경상북도', '010-1234-1234', 20000);
   ```
...
1. 전체 평균 나이
   ...
   User.objects.aggregate(total_avg_age=Avg('age'))
   ```
...
4. 계좌 잔액 중 가장 높은 값
   ...
   ```python
   # orm
   User.objects.order_by('-balance').first()
   user.balance 
   ```
...
3. Article - Comment가 1:N인 경우
   ...
   Article.objects.annotate(
       						...
       						pub_date=Count('comment', filter=Q(comment_created_at_lte='2020-01-01'))
   )
   ```
   ...
   {% for article in articles %}
	<P>이 게시글에 작성된 댓글은 총 {{ articles.total_comment }}개</P>
